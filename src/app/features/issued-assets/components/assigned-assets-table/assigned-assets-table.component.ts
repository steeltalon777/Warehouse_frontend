...
@Component({
            ...
            @for (row of rows(); track row.inventory_subject_id; let i = $index) {
              <tr>
                ...
                <td class="col-actions">
                  <button class="wh-btn wh-btn--secondary btn btn-action" (click)="return.emit(row)">Возврат</button>
                  <button class="wh-btn wh-btn--secondary btn btn-action btn-action--danger" (click)="writeOff.emit(row)">Списание</button>
                </td>
              </tr>
            } @empty {
              <tr>
                <td colspan="6" class="empty-state">Имущество не назначено объекту</td>
              </tr>
            }
          </tbody>
        </table>
  ...
  styles: [`
    ...
    .assets-table-wrapper { display: flex; flex-direction: column; flex: 1; min-height: 0; overflow: hidden; }
    ...
    .data-table th { font-weight: 600; color: #475569; background: #F8FAFC; user-select: none; position: sticky; top: 0; z-index: 2; font-size: 12px; }
